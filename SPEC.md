...
| `/F<uint:n> [<num:size>] Tf` | Select font variant: `n = 2` → bold, `n = 3` → italic, anything else → regular. The optional size operand sets the font size and follows the same rules as `Ts`; omitting it leaves the size unchanged. |
